...
jQuery(document).ready(function(a) {
	if (a('*[data-class*="ExternalLink"]').length) {
		jQuery('*[data-class*="ExternalLink"]').lightbox()
	}
	a(function() {
		a('*[data-class*="ExternalLink"]').attr("href", function(b, c) {
			return c + (c.indexOf("?") != -1 ? "&lightbox[iframe]=true&lightbox[width]=770&lightbox[height]=460" : "?lightbox[iframe]=true&lightbox[width]=770&lightbox[height]=460")
		})
	});
});
*/
...
$("img.lazy").lazyload({
            ...
            load : function()
            {
				...
				jQuery(this).parents('figure').find('figcaption.caption').css('margin-top', h / 2 + 'px');
				jQuery(this).parents('tr').find('td').equalHeights();
            }
        });
$("figure:not(:has(figcaption))").each(function(index, value) {
	$(this).find('img').addClass('nofigc');
});
//$('ol:has(hr)').css({'padding-left':'0'});
